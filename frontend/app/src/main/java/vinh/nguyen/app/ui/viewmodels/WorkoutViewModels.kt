...
import vinh.nguyen.app.utils.WorkoutTTSManager
...
class WorkoutViewModel(application: Application): AndroidViewModel(application) {
    ...
    // TTS Manager
    private val ttsManager = WorkoutTTSManager(getApplication<Application>().applicationContext)

    // TTS enabled flag (can be connected to SharedPreferences later)
    private var _isTTSEnabled = MutableStateFlow(true)
    ...
    fun stopWorkout() {
        ...
        ttsManager.stop() // Stop any ongoing TTS
    ...
    fun reset() {
        if (_state.value.isWorkoutActive) {
            ttsManager.stop() // Stop TTS when resetting
            ...
            try {
                ...
                if (response.isSuccessful) {
                    response.body()?.let { result ->
                        ...
                        // Log rep changes and announce via TTS
                        if (result.rep_count > currentState.repCount) {
                            Log.i(TAG, "REP COUNT: ${result.rep_count}!")

                            // TTS Announcement
                            ttsManager.announceRep(result.rep_count, _isTTSEnabled.value)
                        }
                    }
                } else {
                    consecutiveFailures++
