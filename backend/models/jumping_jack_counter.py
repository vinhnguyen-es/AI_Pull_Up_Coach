...
from config.jumping_jack_config import jumping_jack_config, DebugMode
from utils.logging_utils import logger

from models.base_counter import Counter
from utils.keypoint_utils import extract_ankle_keypoints, calculate_ankle_positions, extract_wrist_keypoints, \
    calculate_wrist_positions
...
class JumpingJackCounter(Counter):
    ...
    def _record_direction_change(self, new_direction: str, ankle_positions: tuple[tuple[float, float], ...],
        ...
        """
        self.direction_history.append((new_direction, time.time(), ankle_positions, wrist_positions))
        self.current_direction = new_direction

        # Log direction changes in debug mode for troubleshooting
        if jumping_jack_config.debug_mode != DebugMode.NON_DEBUG:
            logger.info(f"Direction change: {self.current_direction.upper()}")

    def detect_direction_change(self, ankle_positions: Tuple[Tuple[float, float], ...], wrist_positions: Tuple[Tuple[float, float], ...]) -> tuple[str, int] | tuple[
        str, tuple[float, ...]]:
        """
        ...
        Args:
            ankle_positions: Current wrist-shoulder vertical difference (in pixels)
        ...
        self.position_history.append(ankle_positions + wrist_positions)
    ...
    def _validate_keypoints(self, keypoints: Optional[np.ndarray]) -> tuple[bool, str, None] | tuple[
        ...
        left_ankle, right_ankle, min_confidence = extract_ankle_keypoints(keypoints)
        left_wrist, right_wrist, min_confidence = extract_wrist_keypoints(keypoints)

        if min_confidence is None:
            return False, self.STATUS_INVALID_KEYPOINTS, None

        # Verify detection confidence is sufficient
        if min_confidence < jumping_jack_config.min_confidence:
            return False, self.STATUS_LOW_CONFIDENCE, None

        ankle_positions = calculate_ankle_positions(left_ankle, right_ankle)
        wrist_positions = calculate_wrist_positions(left_wrist, right_wrist)

        return True, None, ankle_positions, wrist_positions
    ...
    def _check_for_rep_completion(self) -> bool:
        ...
        if current_time - self.last_rep_time <= jumping_jack_config.rep_cooldown:
            return False
    ...
    def analyze_pose(self, keypoints: Optional[np.ndarray]) -> Tuple[int, str]:
        ...
        try:
            ...
            is_valid, error_status, ankle_positions, wrist_positions = self._validate_keypoints(keypoints)
            if not is_valid:
                return self.count, error_status

            # Step 2: Analyze movement direction over time
            direction, magnitude = self.detect_direction_change(ankle_positions, wrist_positions)
