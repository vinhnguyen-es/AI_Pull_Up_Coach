"""
Push-up repetition counter with motion-based detection.
...
from config.push_up_config import push_up_config, DebugMode
...
class PushUpCounter(Counter):
    """
    Push-up counter using direction change detection.
    ...
    def _record_direction_change(self, new_direction: str, current_diff: float) -> None:
        ...
        if push_up_config.debug_mode != DebugMode.NON_DEBUG:
    ...
    def _validate_keypoints(self, keypoints: Optional[np.ndarray]) -> Tuple[bool, Optional[str], Optional[float]]:
        ...
        if min_confidence < push_up_config.min_confidence:
    ...
    def _check_for_rep_completion(self) -> bool:
        ...
        if current_time - self.last_rep_time <= push_up_config.rep_cooldown:
            return False
    ...
    def _update_display_status(self, direction: str) -> None:
        ...
        if direction == self.DIRECTION_UP:
            self.status = self.STATUS_PUSHING_UP
    ...
    def reset(self) -> None:
        ...
        logger.info("Push-up counter reset to initial state")