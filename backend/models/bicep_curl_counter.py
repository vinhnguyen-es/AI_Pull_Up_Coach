"""Bicep Curl
Bicep Curl repetition counter with motion-based detection.

This module implements the core Bicep Curl counting algorithm using direction
change detection. It analyzes vertical movement patterns (wrist-shoulder distance)
to identify complete bicep curl repetitions.

ALGORITHM OVERVIEW:
==================
The algorithm uses a state machine approach to detect complete bicep curl motions:
...
- Wrists (keypoints 9, 10): Track vertical movement during bicep curl
...
class BicepCurlCounter:
    ...
    This class maintains state across frames to track bicep curl motion and count
    ...
    STATE VARIABLES:
    ----------------
    count: Number of completed bicep curl reps
    ...
    def arm_movement_type(self):
        ...
        # left_sum = sum(map(abs, map(lambda x: x['left'], recent_positions)))
        # right_sum = sum(map(abs, map(lambda x: x['right'], recent_positions)))
    ...
    def _check_for_rep_completion(self, moving_arm: str) -> bool:
        ...
        REP DEFINITION:
        A complete bicep curl repetition consists of:
