...
from config.sit_up_config import sit_up_config, DebugMode
from utils.logging_utils import logger

from models.base_counter import Counter
from utils.keypoint_utils import extract_knee_shoulder_ankle_keypoints, calculate_shoulder_knee_diff
...
class SitUpCounter(Counter):
    ...
    def __init__(self, config, logger, test_script=False):
        ...
        logger.info("Sit Up counter initialized")
    ...
    def _record_direction_change(self, new_direction: str, current_diff: float) -> None:
        ...
        if sit_up_config.debug_mode != DebugMode.NON_DEBUG:
    ...
    def detect_direction_change(self, current_diff: float) -> Tuple[str, float]:
        ...
        detected_direction = self._classify_movement_direction(movement, exercise="Sit Ups")
    ...
    def _validate_keypoints(self, keypoints: Optional[np.ndarray]) -> Tuple[bool, Optional[str], Optional[float]]:
        ...
        left_knee, right_knee, left_shoulder, right_shoulder, left_ankle, right_ankle, min_confidence = \
            extract_knee_shoulder_ankle_keypoints(keypoints)

        if min_confidence is None:
            return False, self.STATUS_INVALID_KEYPOINTS, None

        # Verify detection confidence is sufficient
        if min_confidence < sit_up_config.min_confidence:
        ...
        sholder_knee_diff = calculate_shoulder_knee_diff(
            left_shoulder, right_shoulder, left_knee, right_knee
        )

        return True, None, sholder_knee_diff
    ...
    def _check_for_rep_completion(self) -> bool:
        ...
        if current_time - self.last_rep_time <= sit_up_config.rep_cooldown:
            return False
        ...
        if movement_range <= sit_up_config.min_movement_range:
            return False

        ##ADD IN OTHER CRITERIA IF NEEDED
    ...
    def analyze_pose(self, keypoints: Optional[np.ndarray]) -> Tuple[int, str]:
        ...
        try:
            ...
            is_valid, error_status, sholder_knee_diff = self._validate_keypoints(keypoints)
            if not is_valid:
                return self.count, error_status

            # Step 2: Analyze movement direction over time
            direction, magnitude = self.detect_direction_change(sholder_knee_diff)
